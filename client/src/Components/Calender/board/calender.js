...
class Calender extends Component {
  ...
  handleMonthChange(direction) {
    ...
    if (displayedMonth === 12 && direction > 0) {
      ...
      this.setState({
        ...
        board: Array(
          moment(
            `${displayedYear}-${displayedMonth + direction}`,
            'YYYY-MM'
          ).daysInMonth()
        )
      });
  ...
  render() {
    const { displayedYear, displayedMonth, startDate, endDate } = this.state;
